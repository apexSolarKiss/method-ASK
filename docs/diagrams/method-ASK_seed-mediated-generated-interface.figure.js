...
(function () {
  ...
  if (wrap && stage) {
    ...
    /* Fit into the band BELOW the top-corner glass panels rather than centring on the
       whole page — the caption + legend occupy the top corners, and a page-centred wide
       figure renders underneath them. Measured from the live panels so it self-adjusts
       to their height at any viewport width; with no panels present both bands are 0 and
       this is identical to the stock behaviour.

       LOCAL REALIZATION, pending a design-system-ASK default. All five DS diagram engines
       currently centre on the full canvasWrap and reserve no panel band; routed as
       `2026-07-18_method-ASK_to_design-system-ASK_diagram-fit-reserves-panel-band`.
       If DS adopts the default, this override becomes redundant and can be dropped. */
    const fit = () => {
      const r = wrap.getBoundingClientRect(), pad = 90, gutter = 26;
      const panelBottoms = [...wrap.querySelectorAll('.caption, .legend')]
        .map(e => e.getBoundingClientRect().bottom - r.top);
      const topBand  = panelBottoms.length ? Math.max(0, Math.max(...panelBottoms)) + gutter : 0;
      const hudTops  = [...wrap.querySelectorAll('.hud')].map(e => r.bottom - e.getBoundingClientRect().top);
      const botBand  = hudTops.length ? Math.max(...hudTops) + gutter : 0;
      const availH   = Math.max(120, r.height - topBand - botBand);
      sc = Math.min((r.width - pad) / vbW, (availH - 20) / vbH, 1.3);
      tx = (r.width - vbW * sc) / 2;
      ty = topBand + (availH - vbH * sc) / 2;
      apply();
    };
