...
- **[studioLeoV](https://a-s-k.studio/studioleov/)** — collaborative expressive image-making under an ASK-authored aperture: ASK sets the substrate, the conditions for making, the body-making frame, and selection + closure; LeoV's live expressive judgment fills the aperture through drawing. The human-executor instance: it separates project-level source of intent from judgment-bearing expressive execution inside the bounded realization span — the aperture-filler can be a model or a person; the primitive is executor-agnostic.
