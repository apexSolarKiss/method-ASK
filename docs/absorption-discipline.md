...
- **ingesting a routed handoff — renaming its `-TBI` suffix to `-ingested` — is not absorbing it.** Ingestion ≠ absorption: that rename is only the ingestion signal and records nothing about disposition. Post-ingestion disposition is the separate recipient-owned classification that follows (absorb / hold / decline / route / withdraw / no-route), recorded in a durable disposition record; the received memo remains byte-immutable, and current lifecycle disposition is carried by the filename marker plus any separate current-status record (see `docs/source-of-intent.md` §Inbound handoff TBI marker). Leaving `-TBI` on a memo until its payload is absorbed conflates the two and makes the ingestion queue unreliable; leaving `-ingested` on a memo whose disposition is durably recorded makes the closure queue unreliable in the same way, one step later. Ingestion is the third of the four events on the full ingest-and-classify path — routing, feeding, ingestion, disposition — modelled in full at that section. **Disposition is the fourth event, and absorption is one possible disposition** among hold, decline, route-elsewhere, withdraw, no-route, mixed closure, and post-ingestion supersession; this doc owns which of them a given artifact earns. A routed artifact may instead exit before ingestion through `-supersededA`, in which case a pre-ingestion retirement disposition is recorded, but the recipient-classification stage is not reached; no absorption occurs and this doc's post-ingestion classification criteria do not apply. (In that model *routing* means cross-surface artifact routing, a different sense from the material-to-layer routing in §Layer destinations below.)
...
```text
-ingested  →  -absorbed · -held · -declined · -withdrawn · -routed ·
              -no-route · -closed · -supersededP
```
...
- **Lossy supersession.** A superseding memo that *reframes* rather than *subsumes* sheds content silently when it says "supersedes prior versions, discard": the new frame can be genuinely better and still fail to absorb load-bearing material from the versions it replaces — and the downstream fold then faithfully absorbs the loss, because it is faithful to the final memo, not to the session. The guard: a superseding memo either carries forward or **explicitly retires** each prior live claim, so nothing exits the chain unnoticed; and a recovery pass, when one runs, re-reads the discarded sources directly rather than trusting the recovery memo's own scoping (the recovery can itself be lossy).
...
- a future `docs/external-handoff-classification.md` if the now-promoted external / domain-authority handoff rule earns its own first-class home on separability grounds — a heavier threshold than the rule-level promotion already made
