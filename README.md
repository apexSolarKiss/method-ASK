...
The methodology layer articulates the cross-project method; the execution-protocol layer encodes calibrated rules that operationalize the method per session; project repos apply the method and execution-protocol lineage to concrete domains.
...
`docs/method.md` carries the primary method articulation. Standalone doctrine docs and method-altitude worked-example explanations have landed in `docs/` and `examples/`; the article-line index lives at `docs/articles.md`. Illustrative diagrams have landed in [`docs/diagrams/`](docs/diagrams/) — a topology / relative-externality map, and bespoke figures for system-ASK (worked example), bounded generativity (doctrine), and the seed-mediated generated interface (doctrine + ontology); the diagrams are illustrative, and repo prose remains source of truth. Lineage remains a candidate for later PRs as substrate earns it.
...
- `docs/diagrams/` — illustrative diagrams (topology; bounded generativity; seed-mediated generated interface + ontology; intent-artifact ontology; activation + lifecycle comparison; relay / feeding / ingestion; adversarial relay graph + PTX projection); not source of truth
- `docs/read/` — generated reading pages for the doctrine documents, the diagram folder's records and the diagram source files; the Markdown stays source of truth
- `tools/read/` — the generator and its drift and link checks for `docs/read/`
